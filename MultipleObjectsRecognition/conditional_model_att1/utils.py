...
def compatibility_func(L,g):
    ...
    images = tf.reduce_sum(gas,1)
    ga = tf.reduce_sum(gas,[2])
    print(ga.shape)
    print(imgs.shape)
    return ga,imgs,images





def save_train_results(images,labels,feats3,config,step,predict,probs):
...
def save_test_results(images,labels,feats3,config,step,predict,probs):
