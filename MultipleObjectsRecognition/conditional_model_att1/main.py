from utils import save_test_results,save_train_results
...
train_images, train_labels = input_data('crop_data_aug/train.tfrecords', batchsize=config.batch_size, isShuffel=True,
                                   flag=True)
test_images, test_labels = input_data('crop_data_aug/test.tfrecords', batchsize=config.batch_size,isShuffel=True,
                                 flag=False)
...
images,feats3, total_loss, cross_entropy_loss, reg_loss, accuracy, train_op, probs, predictions, learning_rate, summary, global_step = \
...
with tf.Session() as sess:
    ...
    for i in range(config.num_step):
        ...
        result = sess.run([images, Y_input, feats3,total_loss, cross_entropy_loss, reg_loss, train_op, probs,predictions,
        ...
        if i and i % 100 == 0:
            ...
            logger.info(
                'step {}: total_loss = {:3.4f}\tlossXent = {:3.4f}\treg_loss = {:3.4f}'.format(
                    result[-2], result[2],result[3],result[4]))

            logger.info(
                'step {}: train_acc = {:3.4f}'.format(
                result[-2], result[-1]))
        if i and i % 500 == 0:
            save_train_results(result[0],result[1],result[2],config,result[-2],result[-4],result[-5])
        ...
        if i and i % 1000 == 0:
            ...
            for k in range(eval_num):
                result = sess.run([images,Y_input,feats3,probs,predictions,global_step,accuracy], feed_dict={is_train:False})
            ...
            save_test_results(result[0], result[1],result[2], config, result[-2], result[-3], result[-4])
