...
def model(config,images,labels,is_train):
    ...
    for idx in range(num_steps):
        ...
        ga_2, init_p2 = compatibility_func(conv4_3, global_feats)
        ga_3, init_p3 = compatibility_func(conv5_3, global_feats)
        ga = tf.concat([ga_2,ga_3],1)
        ...
        with tf.variable_scope("decoder"):
            expanded_output = ga
            logits = decode(config, expanded_output, is_train)  # (batch,11)
        ...
        #labels_oh = tf.one_hot(labels[:,idx+1],config.num_classes)
        cross_entropy = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
            labels = labels[:,idx],
            logits = logits))
        cross_entropies.append(cross_entropy)
        feats2_pictures.append(init_p2)
    ...
    return images,feats2_pictures,feats3_pictures,total_loss,cross_entropy_loss,reg_loss,accuracy,train_op,probs,predictions,learning_rate,summary,global_step
...
def decode(config,expanded_output,is_train):
